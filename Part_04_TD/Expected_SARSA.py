...
class ES_Agent(object):

    def __init__(self, env, maxEpi=100000, gamma = 0.95, epsilon = 0.1, alpha = 0.01):
        self.env = env
        self.maxEpi = maxEpi
        self.gamma = gamma
        self.alpha = alpha
    ...
    def expected(self, S):
        q = [self.Q[(S,i)] for i in range(self.action_no)]
        best = np.argmax(np.array(q))
        ex = (1-self.epsilon)*q[best]
        for i in range(self.action_no):
            ex += (self.epsilon/self.action_no)*q[i]
        return ex

    def learn(self, eval = 1000, maxStep = 100000):
        self.Q = defaultdict(lambda: 0) # Q value.
        writer = SummaryWriter(comment="TDESARSA")
        ...
        for epi in range(1,self.maxEpi+1):
            ...
            while not done:
                ...
                q = self.Q[(S,A)]
                self.Q[(S,A)] = q + self.alpha * (r + self.gamma * self.expected(Sd) - q)
                S = Sd
